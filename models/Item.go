...
type ItemInfo struct{
	...
	DefaultPageId	string `gorm:"type:varchar(255)" json:"defaultpageid"`
	DefaultCatId2	string `gorm:"type:varchar(255)" json:"defaultcatid2"`
	DefaultCatId3	string `gorm:"type:varchar(255)" json:"defaultcatid3"`
	DefaultCatId4   string `gorm:"type:varchar(255)" json:"defaultcatid4"`
...
func (data *Item) GetItemInfo (keyword string)ItemInfo{
	//var data *Item = new(Item)
	var page Page
	var cat1 Catalogs
	var cat2 Catalogs
...
func (data *ItemInfo)GetMenu (keyword string)Menu{
	var menu Menu
	if keyword ==""{
		menu = getMenu(uint(data.Id))
		return menu
	}

	menu.Page = GetPagesByItemId(uint(data.Id),keyword)
	menu.Catalogs, _ = GetCatalogsByItemId(uint(data.Id),keyword)
...
func getMenu(itemId uint)Menu{
	var menu Menu
	menu = getContent(itemId)
return menu
}
func getContent(itemId uint )Menu{
	var page []*Page
	...
	db.Model(Catalogs{}).Where("item_id = ?",itemId).Order("s_number asc , id asc").Find(&catalogs)
	...
	var menu Menu
	menu.Page = GetPagesByItemId(itemId,"")
